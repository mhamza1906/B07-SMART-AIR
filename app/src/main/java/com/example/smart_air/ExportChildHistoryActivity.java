...
public class ExportChildHistoryActivity extends AppCompatActivity {
    private static final String TAG = "ExportChildHistory";
    ...
    private ArrayList<Integer> zoneColors;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        childUserRef.child("parentID").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Log.e(TAG, "Database error when finding ParentID: " + error.getMessage());
            }
        });
        ...
        zoneColors = new ArrayList<>();
        zoneColors.add(Color.parseColor("#00ff00")); // Green
        zoneColors.add(Color.parseColor("#ffff00")); // Yellow
        zoneColors.add(Color.parseColor("#ff0000")); // Red

        TextView months = findViewById(R.id.TxtMonth);
        monthRange = Integer.parseInt(months.getText().toString().trim());
        Button increment = findViewById(R.id.incrementButton2);
        Button decrement = findViewById(R.id.decrementButton2);
        Button generatePDF = findViewById(R.id.PDFButton);
        zoneChart = findViewById(R.id.zonePie);
        ...
        increment.setOnClickListener(v -> {
            ...
            if (n < 6) {
                n = n + 1;
                months.setText(String.valueOf(n));
                updateMonths(n);
                loadZoneData();
            } else {
                Toast.makeText(this, "Maximum 6 months", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        decrement.setOnClickListener(v -> {
            ...
            if (n > 3) {
                ...
                Toast.makeText(this, "Minimum 3 months", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        generatePDF.setOnClickListener(v -> {
            // reset counts each time we generate
            rescueNum = 0;
            controllerAdherence = 0;
            symptoms = 0;
            activityLimit = 0;
            coughWheeze = 0;
            nightWaking = 0;
    ...
    private void loadZoneData() {
        ...
        redCount = 0;
        zoneMap.put("Green", 0);
        zoneMap.put("Yellow", 0);
        zoneMap.put("Red", 0);

        firestore.collection("PEF")
                .document(childID)
                .collection("log")
                .get()
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful() && task.getResult() != null) {
                        ...
                        for (QueryDocumentSnapshot document : task.getResult()) {
                            ...
                            Date compareDate;
                            try {
                                compareDate = format.parse(dateId);
                            } catch (ParseException e) {
                                Log.e(TAG, "Failed to parse PEF date: " + dateId, e);
                                continue;
                            }
                            ...
                            long diffDays = Duration.between(
                                    compareCalendar.toInstant(),
                                    calendar.toInstant()
                            ).toDays();

                            if (diffDays >= 0 && diffDays <= 30L * monthRange) {
                                String zone = (String) document.get("zone");
                                if (zone != null) {
                                    if ("green".equalsIgnoreCase(zone)) {
                                        greenCount++;
                        ...
                        Log.e(TAG, "Failed to load PEF zone data", task.getException());
                    }
                });
    }
    ...
    public void showPieChart(String label, Map<String, Integer> zoneMap, ArrayList<Integer> colors) {
        ...
        // Ensure consistent order: Green, Yellow, Red
        if (zoneMap.containsKey("Green")) {
            zoneEntries.add(new PieEntry(zoneMap.get("Green"), "Green"));
        }
        if (zoneMap.containsKey("Yellow")) {
            zoneEntries.add(new PieEntry(zoneMap.get("Yellow"), "Yellow"));
        }
        if (zoneMap.containsKey("Red")) {
            zoneEntries.add(new PieEntry(zoneMap.get("Red"), "Red"));
        }
    ...
    // ---------- FIRESTORE FETCH CHAIN FOR PDF ----------

    private void fetchRescueFrequency() {
        firestore.collection("medlog")
                .document(childID)
                .collection("log")
                .get()
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful() && task.getResult() != null) {
                        ...
                        for (QueryDocumentSnapshot document : task.getResult()) {
                            ...
                            try {
                                ...
                                Log.e(TAG, "Failed to parse medlog date: " + dateId, e);
                                continue;
                            }
                            ...
                            if (diffDays >= 0 && diffDays <= 30L * monthRange) {
                                Map<String, Object> rescues =
                                        (Map<String, Object>) document.get("rescue");
                                if (rescues != null) {
                                    rescueNum += rescues.size();
                                }
                            }
                        ...
                        Log.e(TAG, "Failed to fetch medlog for rescue", task.getException());
                    }

                    fetchControllerAdherence();
                });
    }

    private void fetchControllerAdherence() {
                    ...
                    if (task.isSuccessful() && task.getResult() != null) {
                        ...
                        for (QueryDocumentSnapshot document : task.getResult()) {
                            ...
                            try {
                                ...
                                Log.e(TAG, "Failed to parse schedule date: " + dateId, e);
                                continue;
                            }
                        ...
                        if (total > 0) {
                            controllerAdherence = (int) Math.round((adhered * 100.0) / total);
                        } else {
                            controllerAdherence = 0;
                        }

                    } else {
                        Log.e(TAG, "Failed to fetch planned schedule", task.getException());
                    }
    ...
    private void fetchDailyCheckIns() {
        firestore.collection("DailyCheckIns")
                .document(childID)
                .collection("log")
                .get()
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful() && task.getResult() != null) {
                        ...
                        for (QueryDocumentSnapshot document : task.getResult()) {
                            ...
                            try {
                                ...
                                Log.e(TAG, "Failed to parse DailyCheckIns date: " + dateId, e);
                                continue;
                            }
                        ...
                        Log.e(TAG, "Failed to fetch DailyCheckIns", task.getException());
                    }

                    fetchTriageIncidents();
                });
    }

    private void fetchTriageIncidents() {
        firestore.collection("triage_incidents")
                .document(childID)
                ...
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful() && task.getResult() != null) {
                        for (QueryDocumentSnapshot document : task.getResult()) {
                            ...
                            if (diffDays >= 0 && diffDays <= 30L * monthRange) {
                                Map<String, Object> redFlags =
                                        (Map<String, Object>) document.get("redFlagsPresent");
                                if (redFlags != null) {
                                    for (Object key : redFlags.keySet()) {
                                        Object value = redFlags.get(key);
                        ...
                        Log.e(TAG, "Failed to fetch triage_incidents", task.getException());
                    }

                    // All data collected -> create PDF
                    createPDF(rescueNum, controllerAdherence, symptoms, triage);
                });
    }
    ...
    public void createPDF(int rescueFreq, int controllerAdherence, int symptomBurden, int triageIncidents) {
        FirebaseDatabase.getInstance().getReference("users").child(childID)
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        try {
                            ...
                            String fileName = "Provider-Report-" + fullName.replace(" ", "_") + "-" + timeStamp + ".pdf";
                            ...
                            document.add(new Paragraph("Provider Report").setBold().setFontSize(20));
                            document.add(new Paragraph("Report for: " + fullName).setFontSize(12).setMarginBottom(20));
                            document.add(new Paragraph("Report of last " + monthRange + " months")
                                    .setBold().setFontSize(16).setMarginBottom(15));
                            ...
                            reportTable.addCell(rescueFreq + " days of rescue use");
                            reportTable.addCell(controllerAdherence + "%");
                            reportTable.addCell(symptomBurden + " problem days");
                            reportTable.addCell(triageIncidents + " notable triage incidents");

                            document.add(reportTable);

                            // Add the zone pie chart image
                            ByteArrayOutputStream stream = new ByteArrayOutputStream();
                            Bitmap zoneBitmap = zoneChart.getChartBitmap();
                            if (zoneBitmap != null) {
                                zoneBitmap.compress(Bitmap.CompressFormat.JPEG, 100, stream);
                                ...
                                ImageData imageData = ImageDataFactory.create(zoneBitmapData);
                                Image zoneImage = new Image(imageData);
                                zoneImage.setHeight(200);
                                zoneImage.setWidth(200);
                                zoneImage.setHorizontalAlignment(CENTER);
                                zoneImage.setMarginTop(20);
                            ...
                            Toast.makeText(ExportChildHistoryActivity.this,
                            ...
                            Toast.makeText(ExportChildHistoryActivity.this,
                                    "Failed to create PDF. Check permissions and storage.",
                                    Toast.LENGTH_LONG).show();
                        } catch (Exception e) {
                            Log.e(TAG, "Unexpected error creating PDF", e);
                            Toast.makeText(ExportChildHistoryActivity.this,
                                    "Unexpected error creating PDF.",
                                    Toast.LENGTH_LONG).show();
                        }
                    }
                    ...
                    public void onCancelled(@NonNull DatabaseError error) {
                        ...
                        Toast.makeText(ExportChildHistoryActivity.this,
                                "Could not fetch child's name for PDF.",
